...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      // Find or create Medusa customer
      ...
      // Find SubscriptionPlan by stripe_price_id
    ...
    case "customer.subscription.created": {
      ...
      // Determine price and map to SubscriptionPlan
      const firstItem = stripeSub.items?.data?.[0]
      const priceId = firstItem?.price?.id
      ...
      // Shipping snapshot from Stripe customer
    ...
    case "invoice.payment_succeeded": {
      ...
      if (!stripeSubId) {
        ...
        stripeSubId =
          ...
          (firstLine?.parent?.subscription_item_details?.subscription as string | undefined) ??
          null
      }
      ...
      if (plan && unitAmount != null && currency) {
        try {
          ...
          if (countryCode) {
            try {
              // Find region that supports this country
          ...
          // Fallback to env variable or first available region
          if (!regionId) {
          ...
          if (!regionId) {
            throw new Error(
              "No region found - please set SUBSCRIPTIONS_REGION_ID or ensure product country matches a region"
            )
          }

          if (!salesChannelId) {
            throw new Error(
              "No sales channel found - please set SUBSCRIPTIONS_SALES_CHANNEL_ID or link product to a sales channel"
            )
          }
          ...
          // 🔍 Step 5: Build order input with all required fields
